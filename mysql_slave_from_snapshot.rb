...
begin
  ...
  original_name = rds_client.describe_db_snapshots({db_snapshot_identifier: my_snapshot})['db_snapshots'][0].db_instance_identifier
rescue
...
# get an resource object for the new db instance, use it to monitor if the db instance is up yet
my_resource = Aws::RDS::Resource.new(client: rds_client)
my_db_instance = my_resource.db_instance(my_name)

my_db_instance.wait_until(max_attempts:30, delay:60) do |instance| 
